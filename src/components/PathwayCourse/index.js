...
function PathwayCourse() {
  ...
  let str = 'Someone humorously remarked that our age can be called “The Plastic Age.” Plastic is a non-biodegradable substance. It does not assimilate with soil and reduces the fertility of the land. It cannot be recycled automatically and harms the life cycle of the earth. Plastic is replacing  traditional use of jute, cotton, or paper materials and is becoming a menace to the living world. Children run the risk of being suffocated while playing with plastic bags. Coloured plastic cause poisoning of the food. Plastic wastes, if not carefully disposed of, very often lead to the choking of the sewage system.'
  ...
  const calculateWpm = () => {
    ...
    const minutes = value.length / 5; // assuming an average of 5 characters per word
    let total = words / minutes * 100
    if (typeof (total) === 'number' && total !== Infinity)
      setCpm(parseInt(total));
  }
    
  const calculateAcc = () => {
    const strWords = str.split(' ')
    const strLength = strWords.length;
    const words = value.split(' ');
    for (let i = 0; i < strLength; i++) {
      if (str.includes(words[i])) {
        if (accuracy > 100) setAccuracy(100)
        else setAccuracy((x) => x + 1)
      }
    }
  }
              ...
              >
                <Grid item xs={12} md={6} sx={{ pl: 2 }}>
                  ...
                  >
                    ...
                    {
                      user?.data?.token && pathwayCourseData.code == 'TYPGRU' &&
                      <>
                        <Button onClick={handleOpen}>Take a Typing Test</Button>
                        ...
                        >
                          <Box sx={style}>
                            <>
                            <Typography variant="h5" sx={{color:"green"}}>Take a quick typing test </Typography>
                              {
                                show === true ?
                                  <textarea style={{ userSelect: 'none', width: '100%', height: '30vh' }} value={str} />
                                  :
                                  <textarea style={{ width: '100%', height: '10vh' }} value="The aim of the wise is not to secure pleasure, but to avoid pain. The complete is more than the sum of its pieces." />
                              }
                              <br />
                              {
                                !show ?
                              ...
                              {
                                show ?
                                  <textarea style={{ width: '100%', height: '20vh', borderRadius:'10px' }} onChange={handleValue} value={value} />
                                  :
                                  <textarea style={{ userSelect: 'none', width: '100%', height: '10vh' }} value="Type here after starting the test." />
                              }
                              <span style={{color:'green'}}> Time : {timer} seconds</span>
                              <br />
                              <br />
                              <span style={{color:'green'}}> Words Per min {cpm}</span>
                              <br />
                              <br />
                              <span style={{color:'green'}}>Accuracy is {accuracy}%</span>
                            </>
                          </Box>
                        </Modal></>
                    }
